...
os.environ['PYOPENGL_PLATFORM'] = 'egl'  # necessary to run headless
...
def process(scene, total_scenes_index, total_scenes_count, save_path):
    ...
    test_framid = os.listdir(os.path.join(args.gt_path, scene, 'color'))
    n_imgs = len(test_framid)
    intrinsic_dir = os.path.join(args.gt_path, scene, 'intrinsic', 'intrinsic_depth.txt')
    cam_intr = np.loadtxt(intrinsic_dir, delimiter=' ')[:3, :3]
    dataset = ScanNetDataset(n_imgs, scene, args.gt_path, args.max_depth)
...
def process_with_single_worker(info_files, save_path):
    ...
    for i, info_file in enumerate(info_files):
        if os.path.isfile(os.path.join(save_path, info_file + '_metrics.json')):
            print('read json then skipping {}'.format(info_file))
            scene = info_file
            temp = json.load(open(os.path.join(save_path, info_file + '_metrics.json'), 'r'))
        else:
...
def main():
    ...
    # data_path = 'results/scene_scannet_neucon49-val-run_fusion_eval_49'
    data_path = 'results/scannet_logs_eval_fusion_eval_54'
    scene_list = os.listdir(data_path)

    # info_files = split_list(info_files, all_proc)  # for ray multi-worker
    # extract all the scene names in data_path
    scene_list = sorted([i.split('.')[0] for i in scene_list if i.endswith('.npz')])
    ...
    metrics = process_with_single_worker(scene_list, save_path=data_path)

    # metrics = {}
    # for r in results:
    #     metrics.update(r)

    rslt_file = os.path.join(data_path, 'metrics.json')
    json.dump(metrics, open(rslt_file, 'w'))

    # display results
    visualize(rslt_file, write_path=os.path.join(data_path, '2d3d_result'))
