...
# All taken from Zeng, except for integrate() and TSDFVolumeTorch
...
class TSDFVolume:
    """Volumetric TSDF Fusion of RGB-D Images. For generate gt.
    """
    ...
    def __init__(self, vol_bnds, voxel_size, use_gpu=True, margin=5):
        ...
        #   vol_bnds in meter, thus div by voxel_siz gets vol_dim in number of voxels
        self._vol_dim = np.round((self._vol_bnds[:, 1] - self._vol_bnds[:, 0]) / self._voxel_size).copy(
            order='C').astype(int)

        # re-adjust, sync the raw vol bound & vol origin into the rounded versions
...
class TSDFVolumeTorch:
    """Volumetric TSDF Fusion of RGB-D Images. For transform and then used in training/testing.
    """
