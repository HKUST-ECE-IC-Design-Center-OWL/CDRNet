...
class DemoDataset(Dataset):
    ...
    def build_list(self):
        with open(os.path.join(self.datapath, 'fragments.pkl'), 'rb') as f:
            metas = pickle.load(f)  # fragments are those 9 frames selected as key frames in between from time to time, there are multiple fragments
            # print('the meta in demo dataset is {}' .format(metas))
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(meta['image_ids']):
            ...
            imgs.append(
                self.read_img(
                    os.path.join(self.datapath, 'color', '{}.jpg'.format(vid))))
